"""
Compare CommonRoad car-follow experiment runs: plots + LaTeX table + terminal summary.

Mirrors scripts/debug/plot_car_follow.py but reads from experiments/commonroad/results/.

Usage:
    # Auto-discover all runs
    python experiments/commonroad/plot_carfollow.py

    # Specific directories
    python experiments/commonroad/plot_carfollow.py results/run1 results/run2

    # Save figure
    python experiments/commonroad/plot_carfollow.py --output comparison.pdf

    # Print LaTeX table
    python experiments/commonroad/plot_carfollow.py --latex

    # Filter by method
    python experiments/commonroad/plot_carfollow.py --human rbf --intervention cbf_single
"""
...
def load_run(run_dir: str) -> dict:
    ...
    # Resolve relative to RESULTS_DIR if needed
...
def extract_metrics(episode: dict, meta: dict) -> dict:
    ...
    d_target = meta.get("target_distance", 20.0)
    d_safe = meta.get("d_safe", 15.0)
    ...
    if len(distance) > 0:
        ...
        d_error_pct = d_error_abs / d_target * 100.0
    ...
    for i, r in enumerate(runs):
        ...
        if d_raw:
            ax_dist.plot(d_raw, color=colours[i], linewidth=1.2,
                         label=labels[i], alpha=0.8)
    d_target = runs[0]["metrics"].get("d_target", 20.0)
    d_safe = runs[0]["metrics"].get("d_safe", 15.0)
    ...
    if output:
        ...
        if backend != "agg":
            ...
            fallback = os.path.join(os.path.dirname(__file__),
                                    "figures", "car_follow_comparison.pdf")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare CommonRoad car-follow experiment runs")
    ...
    parser.add_argument("--human", type=str, default=None,
                        choices=["static", "rbf"],
                        help="Filter runs by human type")
    ...
    parser.add_argument("--experiment", "-e", type=str, default=None,
                        help="Filter runs by experiment name")
...
def main():
    ...
    if args.dirs:
        ...
        if not run_dirs:
            ...
            print("Run run_carfollow.py first, or pass directories explicitly.")
            sys.exit(1)
