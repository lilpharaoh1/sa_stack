"""
Plot the full episode trajectory coloured by intervention magnitude.

- Non-ego agents: trajectory points coloured by P(hidden) darkening
  (same scheme as belief_snapshot).
- Ego agent: trajectory points in purple — lighter for small/no
  intervention, darker for larger intervention magnitude.
- Static obstacles drawn as grey polygons.

Usage:
    python scripts/experiments/plot_intervention_magnitude_scene.py results/my_run.pkl -o scene.png
    python scripts/experiments/plot_intervention_magnitude_scene.py results/my_run.pkl -e 2
"""
...
# Purple range: lightest (no intervention) to darkest (max intervention)
PURPLE_LIGHT = np.array([0.85, 0.75, 0.95])  # pale lavender
PURPLE_DARK = np.array([0.30, 0.0, 0.50])    # deep purple
...
def _purple_colour(t):
    """Interpolate from PURPLE_LIGHT (t=0) to PURPLE_DARK (t=1)."""
    c = (1.0 - t) * PURPLE_LIGHT + t * PURPLE_DARK
    return tuple(c)


def plot_intervention_magnitude_scene(
    ...
    """Plot full-episode trajectories coloured by intervention magnitude.
    ...
    # --- Static obstacles (from last step) ---
    last_sr = steps[-1]
    ...
    # --- Ego trajectory ---
    if len(ego_xy) > 0:
        mag_max = max(ego_magnitudes.max(), 1e-6)
        mag_norm = ego_magnitudes / mag_max

        for j in range(len(ego_xy)):
            colour = _purple_colour(mag_norm[j])
    ...
    # --- Colourbar ---
    n_cb = 256
    purple_colours = [(*_purple_colour(t), 1.0)
                      for t in np.linspace(0, 1, n_cb)]
    purple_cmap = mcolors.ListedColormap(purple_colours)
    norm = mcolors.Normalize(vmin=0.0, vmax=mag_max)
    sm = ScalarMappable(cmap=purple_cmap, norm=norm)
    sm.set_array([])
    fig.colorbar(sm, cax=cbar_ax)
    cbar_ax.set_ylabel("Intervention magnitude", fontsize=9)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot full-episode trajectory coloured by intervention magnitude")
...
def main():
    ...
    fig, ax = plot_intervention_magnitude_scene(
        result, scenario_map,
