...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--live-velocity", action="store_true",
                        help="Show live velocity attention kernel plot during episode")
    parser.add_argument("--live-mcts", action="store_true",
                        help="Show live MCTS tree plotter during episode")
...
def run_single_experiment(config: dict,
    ...
    config["agents"][0]["b_kappa"] = b_kappa
    config["agents"][0]["q_kappa"] = q_kappa
    config["agents"][0]["plot_mcts_tree"] = plot_mcts_tree
    ...
    # Live velocity kernel plotter
    velocity_kernel_plotter = None
    if live_velocity and ego_agent is not None:
        from live.velocity_kernel_plotter import LiveVelocityKernelPlotter
        velocity_kernel_plotter = LiveVelocityKernelPlotter(scenario_map, ego_agent)

    # Live trajectory plotter — auto-enabled for MCTS inference + MCTS ref-controls
    trajectory_plotter = None
    if (plot_interval
            and inference_type.startswith('mcts')
            and ref_controls.startswith('mcts')
            and ego_agent is not None):
        from live.trajectory_plotter import LiveTrajectoryPlotter
        trajectory_plotter = LiveTrajectoryPlotter(scenario_map, ego_agent)
    ...
    for t in range(max_steps):
        ...
        if ego_agent is not None and current_frame is not None:
            ...
            if velocity_kernel_plotter is not None:
                velocity_kernel_plotter.update(record)
            if trajectory_plotter is not None:
                trajectory_plotter.update(record)
    ...
    for fig_num in list(plt.get_fignums()):
        ...
        if fig.get_label() in ('live_awareness', 'live_velocity_kernel',
                                'live_trajectory'):
