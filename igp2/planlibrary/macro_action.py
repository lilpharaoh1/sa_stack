...
class ChangeLane(MacroAction):
    ...
    def check_applicability(state: AgentState, scenario_map: Map, left: bool) -> bool:
        ...
        lane = current_lane.link.successor[0] if in_junction and current_lane.link.successor is not None else current_lane
    ...
    def get_maneuvers(self) -> List[Maneuver]:
        ...
        if not in_junction:
            ...
            if current_lane.length - current_distance > GiveWay.GIVE_WAY_DISTANCE + Maneuver.POINT_SPACING:
                ...
                # logger.debug(f"FollowLane termination_point) {lane_follow_termination}")
                config = ManeuverConfig(config_dict)
                if self.open_loop:
                    man = FollowLane(config, self.agent_id, frame, self.scenario_map)
                else:
            ...
            # logger.debug(f"GiveWay termination_point) {np.array(current_lane.midline.coords[-1])}")
        ...
        # logger.debug(f"Turn termination_point) {self.turn_target}")
        config = ManeuverConfig(config_dict)
        if self.open_loop:
            man = Turn(config, self.agent_id, frame, self.scenario_map)
        else:
